...
app.post('/soap', function m1(req, res){
  ...
  pool.getConnection(function(error, connection){
    ...
    let valuesClause = "( '"+ reqBody.name +"', '"+ reqBody.description +"', '"+ reqBody.price +"' )"

    console.log("Values clause:: ", valuesClause);
    let query = 'INSERT INTO soaps(`name`, `description`, `price`) values ' + valuesClause;


    console.log("QUERY ::", query);
    connection.query(query, reqBody, function(err, results, fields){
